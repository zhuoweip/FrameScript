...
/// </summary>
	public class PathMagicEditor : Editor
	{
		...
		/// <summary>
		/// Internally stores the preview transform check
		/// </summary>
		private bool updateTransformPreview = true;
		...
		public override void OnInspectorGUI ()
		{
			...
			if (previewFoldout) {
				...
				updateTransformPreview = EditorGUILayout.ToggleLeft (new GUIContent ("Update transform", "If true, the connected transform will be moved and rotated in the simulation"), updateTransformPreview);
        
				for (int i = 0; i < targets.Length; i++) {
					if (!((PathMagic)targets [i]).isPlaying && updateTransformPreview && ((PathMagic)targets [i]).target != null) {
			...
			if (utilityFoldout) {
				...
				if (serializedObject.isEditingMultipleObjects) {
					...
					EditorGUILayout.BeginHorizontal ();
					if (GUILayout.Button (new GUIContent ("Normalize path transform", "Normalize this transform and update all waypoints"), GUILayout.Width ((EditorGUIUtility.currentViewWidth - 30f) / 2f))) {
					...
					if (GUILayout.Button (new GUIContent ("Center on waypoints", "Updates this transform position to center of its waypoints and update all waypoints"), GUILayout.Width ((EditorGUIUtility.currentViewWidth - 30f) / 2f))) {
					...
					EditorGUILayout.BeginHorizontal ();
					if (GUILayout.Button (new GUIContent ("Look to forward", "Updates all waypoints to look forward in the path"), GUILayout.Width ((EditorGUIUtility.currentViewWidth - 30f) / 2f))) {
					...
					if (GUILayout.Button (new GUIContent ("Look to center", "Updates all waypoints to look at the center of the path"), GUILayout.Width ((EditorGUIUtility.currentViewWidth - 30f) / 2f))) {
		...
		{ 
			...
			for (int i = 0; i < waypoints.arraySize; i++) {
				...
				if (!updateTransformPreview || pm.FindProperty ("target").objectReferenceValue == null) {
