Shader "Monotype/DistanceFieldOutline" {
    Properties {
    ...
    SubShader
    {
        ...
        Pass {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float ic = 0.5 + _OutsideCut * _OutlineWidth;
                float oc = ic - (_InsideCut - _OutsideCut);
                float alpha = (clamp(dist.a, oc, ic) - oc) / (ic - oc);

                return float4(_OutlineColor.rgb, alpha * _OutlineColor.a * i.color.a);
            }
            ENDCG
        }
